  ...
  module Generators
    class BlogGenerator < Rails::Generators::Base
      ...
      def generate_migration
        ...
        destination   = File.expand_path('db/migrate/02_create_tags.rb', self.destination_root)
        migration_dir = File.dirname(destination)
        destination   = self.class.migration_exists?(migration_dir, 'create_tags')

        if destination
          puts "\e[0m\e[31mFound existing create_tags migration. Remove it if you want to regenerate.\e[0m"
        else
          migration_template 'db/migrate/02_create_tags.rb', 'db/migrate/create_tags.rb'
        end
      end
